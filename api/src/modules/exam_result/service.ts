...
export class ExamResultService
  extends BaseService<ExamResultEntity, ExamResultReqI, ExamResultResI>
  implements ExamResultServiceI
{
  ...
  async findAndFilter(userId: number, examGroupId: number) {
    const query = this.repository
      ...
      .andWhere('e.active')
      .groupBy(
        'er.id, er.exam_id, er.user_id, er.status, er.device, er.number_of_correct_answer, er.score, er.created_at',
      );
  ...
  async create(examResultReq: ExamResultReqI): Promise<ExamResultResI> {
    try {
      ...
      const newAnswers: AnswerResI[] = await Promise.all(
        answersWithExamResultId.map((q) => this.answerService.create(q)),
      );

      // calculate number_of_correct_answer and update to the record
      const number_of_correct_answer: number =
        this.countCorrectAnswer(newAnswers);

      const queryToUpdate: UpdateQueryBuilder<ExamResultEntity> =
        this.repository
          .createQueryBuilder(this.getTableName())
          .update()
      ...
      if (!result || !Array.isArray(result.raw) || result.raw.length === 0) {
        throw new InternalServerErrorException(
          'Failed to update exam result with answers',
        );
      }
      ...
      // handle duplicate exam submission
      if (
        error.code === '23505' &&
        error.constraint === 'IDX_exam_result_exam_user_active'
      ) {
  ...
  async updateOne(
    id: number,
    examResultReq: ExamResultReqI,
  ): Promise<ExamResultResI> {
    const userId: number | null = this.getAuthenticatedUserId();
    const { questions: updatingQuestions, ...rest } = examResultReq;

    const updatedQuestions: AnswerResI[] =
      await this.answerService.updateMany(updatingQuestions);
    const number_of_correct_answer: number =
      this.countCorrectAnswer(updatedQuestions);
