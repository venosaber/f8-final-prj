  ...
  implements UserServiceI
{
  ...
  async changePassword(data: ChangePasswordReqI) {
    ...
    if (!saltRounds)
      throw new InternalServerErrorException('Not found salt rounds in config');
  ...
  protected getPublicColumns(): string[] {
    return super
      .getPublicColumns()
      .filter((column) => !['password', 'avatar', 'id'].includes(column));
  }
